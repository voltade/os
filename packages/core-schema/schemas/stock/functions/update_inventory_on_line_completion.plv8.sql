drop function if exists internal.update_inventory_on_line_status_change () cascade;

create or replace function internal.update_inventory_on_line_status_change () returns trigger as $plv8$
...
// src/schemas/stock/functions/update_inventory_on_line_completion.plv8.ts
function update_inventory_on_line_status_change(OLD, NEW) {
  plv8.elog(NOTICE, `
  ...
  const operationResult = db.select({
      ...
      }).from(stockOperationTable).innerJoin(stockOperationTypeTable, eq(stockOperationTable.type_id, stockOperationTypeTable.id)).where(eq(stockOperationTable.id, NEW.stock_operation_id)).limit(1).execute();
  if (operationResult.length === 0) {
    plv8.elog(WARNING, `Stock operation not found for operation_id=${NEW.stock_operation_id}`);
    return NEW;
  }
  const operationRecord = operationResult[0];
  if (!operationRecord) {
    plv8.elog(WARNING, `Invalid operation record for operation_id=${NEW.stock_operation_id}`);
    return NEW;
  }
  plv8.elog(NOTICE, `
  Operation details:
  ...
  const safeUpsertInventory = (warehouseId, locationId, onHandDelta, reservedDelta, incomingDelta) => {
    plv8.execute("SELECT internal.upsert_inventory_quantities($1, NULL, $2, $3, $4, $5, $6)", [
      NEW.product_id,
      warehouseId,
      locationId,
      onHandDelta,
  ...
  if (NEW.status === "Reserved" /* RESERVED */ && (!OLD || OLD.status === null || OLD.status === "Pending" /* PENDING */)) {
    plv8.elog(NOTICE, "Processing reservation for", `operation type: ${operationRecord.name}`);
    switch (operationRecord.name) {
      case "Import" /* IN */:
      case "Manufacture" /* MO */:
      case "Return" /* RTN */:
        safeUpsertInventory(operationRecord.destination_warehouse_id, operationRecord.destination_location_id, 0, 0, quantityDelta);
        break;
      case "Export" /* OUT */:
      case "Sale" /* POS */:
        safeUpsertInventory(operationRecord.source_warehouse_id, operationRecord.source_location_id, -quantityDelta, quantityDelta, 0);
        break;
      case "Transfer" /* TRF */:
        safeUpsertInventory(operationRecord.source_warehouse_id, operationRecord.source_location_id, -quantityDelta, quantityDelta, 0);
        safeUpsertInventory(operationRecord.destination_warehouse_id, operationRecord.destination_location_id, 0, 0, quantityDelta);
    ...
    plv8.elog(NOTICE, "Processing completion for", `operation type: ${operationRecord.name}`);
    switch (operationRecord.name) {
      case "Import" /* IN */:
      case "Manufacture" /* MO */:
      case "Return" /* RTN */:
        safeUpsertInventory(operationRecord.destination_warehouse_id, operationRecord.destination_location_id, quantityDelta, 0, -quantityDelta);
        break;
      case "Export" /* OUT */:
      case "Sale" /* POS */:
        safeUpsertInventory(operationRecord.source_warehouse_id, operationRecord.source_location_id, 0, -quantityDelta, 0);
        break;
      case "Transfer" /* TRF */:
        safeUpsertInventory(operationRecord.source_warehouse_id, operationRecord.source_location_id, 0, -quantityDelta, 0);
        safeUpsertInventory(operationRecord.destination_warehouse_id, operationRecord.destination_location_id, quantityDelta, 0, -quantityDelta);
    ...
    plv8.elog(NOTICE, "Processing cancellation from", `previous status: ${OLD.status}`);
    if (OLD.status === "Reserved" /* RESERVED */) {
      ...
      switch (operationRecord.name) {
        ...
        case "Manufacture" /* MO */:
        case "Return" /* RTN */:
          safeUpsertInventory(operationRecord.destination_warehouse_id, operationRecord.destination_location_id, -quantityDelta, 0, 0);
          break;
        case "Export" /* OUT */:
        case "Sale" /* POS */:
          safeUpsertInventory(operationRecord.source_warehouse_id, operationRecord.source_location_id, quantityDelta, 0, 0);
          break;
        case "Transfer" /* TRF */:
          safeUpsertInventory(operationRecord.source_warehouse_id, operationRecord.source_location_id, quantityDelta, 0, 0);
          safeUpsertInventory(operationRecord.destination_warehouse_id, operationRecord.destination_location_id, -quantityDelta, 0, 0);
      ...
      plv8.elog(WARNING, `Unhandled status transition from ${OLD.status} to ${NEW.status} for operation type ${operationRecord.name}`);
      return NEW;
    }
  } else {
    plv8.elog(WARNING, `Unhandled status transition from ${OLD?.status ?? null} to ${NEW.status} for operation type ${operationRecord.name}`);
    return NEW;
  }
  plv8.elog(NOTICE, `
...
return update_inventory_on_line_status_change(OLD, NEW);
