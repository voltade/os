drop function if exists internal.update_inventory_on_line_item_status_change () cascade;

create or replace function internal.update_inventory_on_line_item_status_change () returns trigger as $plv8$
...
// src/schemas/stock/functions/update_inventory_on_line_item_completion.plv8.ts
function update_inventory_on_line_item_status_change(OLD, NEW) {
  plv8.elog(NOTICE, `
  ...
  const operationResult = db.select({
      ...
      }).from(stockOperationTable).innerJoin(stockOperationTypeTable, eq(stockOperationTable.type_id, stockOperationTypeTable.id)).innerJoin(stockOperationLineTable, eq(stockOperationTable.id, stockOperationLineTable.stock_operation_id)).where(eq(stockOperationTable.id, NEW.stock_operation_line_id)).limit(1).execute();
  if (operationResult.length === 0) {
    plv8.elog(WARNING, `Stock operation or line not found for line_id=${NEW.stock_operation_line_id}`);
    return NEW;
  }
  const operationLineRecord = operationResult[0];
  if (!operationLineRecord) {
    plv8.elog(WARNING, `Invalid operation line record for line_id=${NEW.stock_operation_line_id}`);
    return NEW;
  }
  plv8.elog(NOTICE, `
  ...
  if (NEW.status === "Reserved" /* RESERVED */ && (!OLD || OLD.status === null || OLD.status === "Pending" /* PENDING */)) {
    plv8.elog(NOTICE, "Processing reservation for", `operation type: ${operationLineRecord.name}`);
    switch (operationLineRecord.name) {
      case "Import" /* IN */:
      case "Manufacture" /* MO */:
      case "Return" /* RTN */:
        safeUpsertInventory(operationLineRecord.destination_warehouse_id, operationLineRecord.destination_location_id, 0, 0, quantityDelta);
        break;
      case "Export" /* OUT */:
      case "Sale" /* POS */:
        safeUpsertInventory(operationLineRecord.source_warehouse_id, operationLineRecord.source_location_id, -quantityDelta, quantityDelta, 0);
        break;
      case "Transfer" /* TRF */:
        safeUpsertInventory(operationLineRecord.source_warehouse_id, operationLineRecord.source_location_id, -quantityDelta, quantityDelta, 0);
        safeUpsertInventory(operationLineRecord.destination_warehouse_id, operationLineRecord.destination_location_id, 0, 0, quantityDelta);
  ...
  } else if (NEW.status === "Completed" /* COMPLETED */ && (!OLD || OLD.status === null || OLD.status !== "Completed" /* COMPLETED */)) {
    plv8.elog(NOTICE, "Processing line item completion for", `operation type: ${operationLineRecord.name},`, `stock_unit_id=${NEW.stock_unit_id}`);
    switch (operationLineRecord.name) {
      case "Import" /* IN */:
      case "Manufacture" /* MO */:
      case "Return" /* RTN */:
        safeUpsertInventory(operationLineRecord.destination_warehouse_id, operationLineRecord.destination_location_id, quantityDelta, 0, -quantityDelta);
        break;
      case "Export" /* OUT */:
      case "Sale" /* POS */:
        safeUpsertInventory(operationLineRecord.source_warehouse_id, operationLineRecord.source_location_id, 0, -quantityDelta, 0);
        break;
      case "Transfer" /* TRF */:
        safeUpsertInventory(operationLineRecord.source_warehouse_id, operationLineRecord.source_location_id, 0, -quantityDelta, 0);
        safeUpsertInventory(operationLineRecord.destination_warehouse_id, operationLineRecord.destination_location_id, quantityDelta, 0, -quantityDelta);
  ...
  } else if (NEW.status === "Cancelled" /* CANCELLED */ && OLD && OLD.status !== null && OLD.status !== "Cancelled" /* CANCELLED */) {
    plv8.elog(NOTICE, "Processing line item cancellation from", `previous status: ${OLD.status},`, `stock_unit_id=${NEW.stock_unit_id}`);
    if (OLD.status === "Reserved" /* RESERVED */) {
      switch (operationLineRecord.name) {
        case "Import" /* IN */:
        case "Manufacture" /* MO */:
        case "Return" /* RTN */:
    ...
    } else if (OLD.status === "Completed" /* COMPLETED */) {
      switch (operationLineRecord.name) {
        case "Import" /* IN */:
        case "Manufacture" /* MO */:
        case "Return" /* RTN */:
          safeUpsertInventory(operationLineRecord.destination_warehouse_id, operationLineRecord.destination_location_id, -quantityDelta, 0, 0);
          break;
        case "Export" /* OUT */:
        case "Sale" /* POS */:
          safeUpsertInventory(operationLineRecord.source_warehouse_id, operationLineRecord.source_location_id, quantityDelta, 0, 0);
          break;
        case "Transfer" /* TRF */:
          safeUpsertInventory(operationLineRecord.source_warehouse_id, operationLineRecord.source_location_id, quantityDelta, 0, 0);
          safeUpsertInventory(operationLineRecord.destination_warehouse_id, operationLineRecord.destination_location_id, -quantityDelta, 0, 0);
      ...
      plv8.elog(WARNING, `Unhandled status transition from ${OLD.status} to ${NEW.status} for operation type ${operationLineRecord.name}`);
      return NEW;
    }
  } else {
    plv8.elog(WARNING, `Unhandled status transition from ${OLD?.status ?? null} to ${NEW.status} for operation type ${operationLineRecord.name}`);
    return NEW;
  }
  plv8.elog(NOTICE, `
...
return update_inventory_on_line_item_status_change(OLD, NEW);
