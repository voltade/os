...
var stockOperationTypeEnum = internalSchema.enum("stock_operation_type_enum", enumToPgEnum(StockOperationType));
...
var stockOperationStatusEnum = internalSchema.enum("stock_operation_status_enum", enumToPgEnum(StockOperationStatus));
...
var stockOperationLineStatusEnum = internalSchema.enum("stock_operation_line_status_enum", enumToPgEnum(StockOperationLineStatus));
...
// src/schemas/stock/tables/stock_operation_type.ts
var stockOperationTypeTable = internalSchema.table("stock_operation_type", {
  ...DEFAULT_COLUMNS,
...
// src/schemas/stock/tables/warehouse.ts
var warehouseTable = internalSchema.table("stock_warehouse", {
  ...DEFAULT_COLUMNS,
...
// src/schemas/stock/tables/warehouse_location.ts
var warehouseLocationTable = internalSchema.table("stock_warehouse_location", {
  ...DEFAULT_COLUMNS,
...
// src/schemas/stock/tables/stock_operation.ts
var stockOperationTable = internalSchema.table("stock_operation", {
  ...DEFAULT_COLUMNS,
  name: text().notNull(),
  description: text(),
...
// src/schemas/stock/tables/stock_operation_line.ts
var stockOperationLineTable = internalSchema.table("stock_operation_line", {
  ...DEFAULT_COLUMNS,
...
// src/schemas/stock/tables/stock_unit.ts
var stockUnitTable = internalSchema.table("stock_unit", {
  ...DEFAULT_COLUMNS,
  product_id: integer().notNull(),
...
// src/schemas/stock/tables/stock_operation_line_item.ts
var stockOperationLineItemTable = internalSchema.table("stock_operation_line_item", {
  ...DEFAULT_COLUMNS,
  ...
  uniqueIndex("stock_operation_line_item_reference_idx").on(table.reference_id).where(sql`reference_id IS NOT NULL`)
]);
...
// src/schemas/stock/functions/update_stock_operation_lines.plv8.ts
function update_stock_operation_lines(OLD, NEW) {
  plv8.elog(NOTICE, `
  ...
  const operationTypeResult = db.select({ name: stockOperationTypeTable.name }).from(stockOperationTypeTable).where(eq(stockOperationTypeTable.id, NEW.type_id)).execute();
  if (operationTypeResult.length === 0) {
    throw new Error(`Stock operation type not found for type_id=${NEW.type_id}`);
  }
  ...
  if (NEW.status === "Created" && (!OLD || OLD.status === null || OLD.status === "Draft")) {
    plv8.elog(NOTICE, `Processing operation creation for type: ${operationTypeName}`);
    switch (operationTypeName) {
      ...
      case "Repair":
        break;
      default:
        plv8.elog(WARNING, `Unknown operation type: ${operationTypeName}`);
    }
  } else if (NEW.status === "Reserved" && (!OLD || OLD.status === null || ["Created", "Assigned", "Pending", "Approved"].includes(OLD.status))) {
    plv8.elog(NOTICE, `Processing operation reservation for type: ${operationTypeName}`);
    const invalidLines = db.select({ id: stockOperationLineTable.id }).from(stockOperationLineTable).where(and(eq(stockOperationLineTable.stock_operation_id, NEW.id), or(isNull(stockOperationLineTable.planned_quantity), lte(stockOperationLineTable.planned_quantity, "0")))).execute();
    if (invalidLines.length > 0) {
      throw new Error("All operation lines must have valid positive planned quantities before reservation");
    }
    db.update(stockOperationLineTable).set({ status: "Reserved" /* RESERVED */ }).where(and(eq(stockOperationLineTable.stock_operation_id, NEW.id), eq(stockOperationLineTable.status, "Pending" /* PENDING */))).execute();
    switch (operationTypeName) {
      case "Import":
        break;
      case "Manufacture":
    ...
    plv8.elog(NOTICE, `Processing operation start for type: ${operationTypeName}`);
    if (OLD?.status && !["Reserved", "Approved"].includes(OLD.status)) {
      throw new Error("Operation must be Reserved or Approved before it can be processed");
    }
    switch (operationTypeName) {
      case "Import":
        break;
    ...
    plv8.elog(NOTICE, `Processing operation completion for type: ${operationTypeName}`);
    switch (operationTypeName) {
      case "Import":
        break;
      case "Manufacture":
    ...
    plv8.elog(NOTICE, `Processing operation cancellation for type: ${operationTypeName}`);
    const operationLines = db.select({ id: stockOperationLineTable.id }).from(stockOperationLineTable).where(eq(stockOperationLineTable.stock_operation_id, NEW.id)).execute();
    if (operationLines.length > 0) {
      const lineIds = operationLines.map((line2) => line2.id);
      db.update(stockOperationLineItemTable).set({ status: "Cancelled" /* CANCELLED */ }).where(and(inArray(stockOperationLineItemTable.stock_operation_line_id, lineIds), ne(stockOperationLineItemTable.status, "Completed" /* COMPLETED */))).execute();
      db.update(stockOperationLineTable).set({ status: "Cancelled" /* CANCELLED */ }).where(and(eq(stockOperationLineTable.stock_operation_id, NEW.id), ne(stockOperationLineItemTable.status, "Completed" /* COMPLETED */))).execute();
    }
    switch (operationTypeName) {
      case "Import":
        break;
    ...
    plv8.elog(NOTICE, `Processing operation finalization for type: ${operationTypeName}`);
    if (OLD?.status !== "Completed") {
      throw new Error("Operation must be Completed before it can be marked as Done");
    }
    switch (operationTypeName) {
      case "Import":
        break;
    ...
    plv8.elog(NOTICE, `No special handling required for status transition: ${OLD?.status} -> ${NEW.status}`);
  }
  plv8.elog(NOTICE, `
...
return update_stock_operation_lines(OLD, NEW);

$plv8$ language plv8 volatile strict;
