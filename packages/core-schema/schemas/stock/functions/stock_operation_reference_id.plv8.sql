drop function if exists internal.set_stock_operation_reference_id () cascade;

create or replace function internal.set_stock_operation_reference_id () returns trigger as $plv8$
...
// src/schemas/stock/tables/stock_operation_sequence.ts
var stockOperationSequenceTable = internalSchema.table("stock_operation_sequence", {
  ...DEFAULT_COLUMNS,
...
// src/schemas/stock/functions/stock_operation_reference_id.plv8.ts
function set_stock_operation_reference_id(NEW) {
  plv8.elog(NOTICE, `
  Setting reference ID for stock operation:
    ...
    type_id: ${NEW.type_id}`);
  if (NEW.source_warehouse_id === null && NEW.destination_warehouse_id === null) {
    throw new Error(`Both source_warehouse_id and destination_warehouse_id are null for operation_id=${NEW.id}`);
  }
  let sourceWarehouse;
  let destinationWarehouse;
  if (NEW.source_warehouse_id !== null) {
    const warehouseResult = db.select().from(warehouseTable).where(eq(warehouseTable.id, NEW.source_warehouse_id)).limit(1).execute();
    sourceWarehouse = warehouseResult?.[0];
    if (!sourceWarehouse) {
      throw new Error(`Warehouse not found with source_warehouse_id=${NEW.source_warehouse_id}`);
    }
  }
  if (NEW.destination_warehouse_id !== null) {
    const warehouseResult = db.select().from(warehouseTable).where(eq(warehouseTable.id, NEW.destination_warehouse_id)).limit(1).execute();
    destinationWarehouse = warehouseResult?.[0];
    if (!destinationWarehouse) {
      throw new Error(`Warehouse not found for destination_warehouse_id=${NEW.destination_warehouse_id}`);
    }
  }
  const operationTypeResult = db.select().from(stockOperationTypeTable).where(eq(stockOperationTypeTable.id, NEW.type_id)).execute();
  ...
  const warehouseRef = operationType.name === "Export" /* OUT */ || operationType.name === "Sale" /* POS */ ? sourceWarehouse : destinationWarehouse;
  if (!warehouseRef) {
    throw new Error(`Warehouse is undefined for operation type ${operationType.name} with type_id=${NEW.type_id}`);
  }
  const updateResult = db.update(stockOperationSequenceTable).set({
        sequence_number: sql`${stockOperationSequenceTable.sequence_number} + 1`
      }).where(and(eq(stockOperationSequenceTable.warehouse_id, warehouseRef.id), eq(stockOperationSequenceTable.type_id, operationType.id))).returning({
        sequence_number: stockOperationSequenceTable.sequence_number
      }).execute();
  let seqNo;
  if (updateResult.length === 0) {
    seqNo = 1;
    plv8.elog(NOTICE, `Creating new sequence for warehouse_id=${warehouseRef.id}, type_id=${NEW.type_id}, starting at ${seqNo}`);
    db.insert(stockOperationSequenceTable).values({
    ...
    plv8.elog(NOTICE, `Updated existing sequence to ${seqNo} for warehouse_id=${warehouseRef.id}, type_id=${NEW.type_id}`);
  }
  const paddedSeq = seqNo.toString().padStart(5, "0");
  const referenceId = `${warehouseRef.code}/${operationType.code}/${paddedSeq}`;
  plv8.elog(NOTICE, `Generated reference ID: ${referenceId} for stock operation`);
  NEW.reference_id = referenceId;
  return NEW;
}
return set_stock_operation_reference_id(NEW);
