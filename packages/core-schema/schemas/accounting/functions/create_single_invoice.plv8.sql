drop function if exists account.create_single_invoice (p_order_id integer, p_type text, p_amount numeric) cascade;

create or replace function account.create_single_invoice (p_order_id integer, p_type text, p_amount numeric) returns integer as $plv8$
...
var paymentTypeEnum = accountingSchema.enum("account_payment_type_enum", ["Inbound", "Outbound"]);
var paymentPartnerType = accountingSchema.enum("account_payment_partner_type_enum", ["Customer", "Vendor", "Employee", "Other"]);
var paymentTermLineValueTypeEnum = accountingSchema.enum("account_payment_term_line_value_type_enum", ["Fixed", "Percentage"]);
var paymentTermLineDelayTypeEnum = accountingSchema.enum("account_payment_term_line_delay_type_enum", [
...
var orderTable = salesSchema.table("order", {
  ...
  check("order_partner_or_contact_id_check", sql`num_nonnulls(${table.partner_id}, ${table.contact_id}) > 0`),
  foreignKey({
...
function create_single_invoice(p_order_id, p_type, p_amount) {
  ...
  const orderLines = db.select().from(orderLineTable).where(eq(orderLineTable.order_id, order.id)).execute();
  const contact_id = order.contact_id;
  plv8.elog(NOTICE, `Contact ID: ${contact_id}`);
  const inserted = db.insert(journalEntryTable).values({
...
return create_single_invoice(p_order_id, p_type, p_amount);
